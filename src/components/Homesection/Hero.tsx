    ...
    >
      ...
      <div className="relative z-10 w-full max-w-5xl pt-12 sm:pt-20 pb-8 sm:pb-12">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 mb-12 sm:mb-16">
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.6 }} whileHover={{ scale: 1.05 }}>
            <MainButton href="/about">Learn More About Us</MainButton>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.7 }} whileHover={{ scale: 1.05 }}>
            <MainButton href="/products" variant="secondary">Explore Our Products</MainButton>
          </motion.div>
        </div>
...
//           ctx.fillStyle = `hsl(${(hue + x + y) % 360}, 80%, 60%)`;
...
// <canvas ref={canvasRef} className="absolute inset-0 z-[-2] w-full h-full opacity-5" />
