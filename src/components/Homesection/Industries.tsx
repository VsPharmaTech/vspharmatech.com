...
interface Industry {
  title: string;
  image: string;
  category: "healthcare" | "miscellaneous";
}
...
export default function Industries() {
  const [activeCategory, setActiveCategory] = useState<"healthcare" | "miscellaneous">("healthcare");
    ...
    <section className="relative py-12 sm:py-16 md:py-20 lg:py-24 xl:py-32 px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 overflow-hidden">
      <div className="relative z-10 container mx-auto">
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-gray-600 text-base sm:text-lg text-center max-w-3xl mx-auto mt-2 sm:mt-3 mb-6 sm:mb-8 md:mb-10">
          Delivering Blow-Fill-Seal systems tailored to meet the unique needs of pharmaceutical and packaging industries worldwide.
        </motion.p>
          ...
          {isMobile ? (
            <div className="relative w-full sm:w-auto">
              <select
                ...
                onChange={(e) => setActiveCategory(e.target.value as "healthcare" | "miscellaneous")}
              >
                {["healthcare", "miscellaneous"].map((category) => (
              ...
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                <img src="/dropdown.png" alt="Dropdown Arrow" className="w-6 h-6" loading="lazy" />
              </div>
            </div>
          ) : (
            <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
              {["healthcare", "miscellaneous"].map((category) => (
                <button
                  ...
                  onClick={() => setActiveCategory(category as "healthcare" | "miscellaneous")}
                >
                  {category.charAt(0).toUpperCase() + category.slice(1)} Sector
                </button>
              ))}
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`grid gap-4 sm:gap-6 place-items-center lg:px-40 ${
              isMobile ? "grid-cols-2" : "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
            }`}
          >
            {industries
                ...
                >
                  ...
                  {isMobile && (
                    <div className="absolute inset-0 bg-gradient-to-t from-black/20">
                      <h3 className="absolute bottom-2 sm:bottom-4 left-2 sm:left-4 right-2 sm:right-4 text-center text-white text-base md:text-lg font-roboto font-semibold leading-relaxed transition-all duration-300">
                        {industry.title}
                      </h3>
                    </div>
                  )}
